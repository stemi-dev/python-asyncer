  ...
  const withLoopLimiters = withAsyncAwait.reduce((all, l) => {
    if (l.loop) {
      ...
      const out = [
        ...
        __spreadProps(__spreadValues({}, l), { line: `if ${varName} >= ${maxIterations}:`, indent: l.indent + indents, sob: true }),
        __spreadProps(__spreadValues({}, l), { line: maxIterError(varName), indent: l.indent + indents * 2, sob: false }),
        __spreadProps(__spreadValues({}, l), { line: `${varName} = ${varName} + 1`, indent: l.indent + indents, sob: false })
      ];
      return [...all, ...out];
    }
    return [...all, l];
